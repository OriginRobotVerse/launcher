...
export function ChatPanel({ robot }: ChatPanelProps) {
  const glove = useGlove();
  const [input, setInput] = useState("");
  const scrollRef = useRef<HTMLDivElement>(null);

  // Track compaction summary indices so we can hide them from the timeline
  const compactionIndices = useRef<Set<number>>(new Set());
  const wasCompacting = useRef(false);

  useEffect(() => {
    if (glove.isCompacting && !wasCompacting.current) {
      // Compaction just started — mark current timeline length
      wasCompacting.current = true;
    } else if (!glove.isCompacting && wasCompacting.current) {
      // Compaction just ended — the last agent_text entry is the summary
      wasCompacting.current = false;
      for (let i = glove.timeline.length - 1; i >= 0; i--) {
        if (glove.timeline[i].kind === "agent_text") {
          compactionIndices.current.add(i);
          break;
        }
      }
    }
  }, [glove.isCompacting, glove.timeline.length]);

  const isCompactionEntry = useCallback(
    (_entry: TimelineEntry, index: number) => compactionIndices.current.has(index),
    [],
  );
      ...
      >
        {glove.timeline.length === 0 && !glove.streamingText && (
          ...
          >
            ...
            <div style={{ textAlign: "center" }}>
              ...
              >
                Tell me what to do with the {robot.name}. Try &quot;list
                available policies&quot;, &quot;start the walk policy&quot;, or
        ...
        <Render
          ...
          renderMessage={({ entry, index }) =>
            isCompactionEntry(entry, index) ? null : <MessageBubble entry={entry} />
          }
          renderStreaming={({ text }) =>
            glove.isCompacting ? null : (
          ...
          <input
            ...
            placeholder={
              glove.busy
                ...
                : `Tell the ${robot.name} what to do...`
            }
            style={{
              flex: 1,
